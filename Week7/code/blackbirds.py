...
# using a regular expression pattern to find all of the Kingdom, Phylum and Species at one time
found_matches = re.findall(r"\bKingdom\b\s+(\w+)\b[\s\S]*?\bPhylum\b\s+(\w+)\b[\s\S]*?\bSpecies\b\s+(\w+\s+\w+)\b", text)
